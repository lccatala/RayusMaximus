...
void HelloVulkan::loadModel(const std::string& filename, glm::mat4 transform)
{
  ...
  VkBufferUsageFlags rayTracingFlags = // Also used for building acceleration structures
...
auto HelloVulkan::objectToVkGeometryKHR(const ObjModel& model)
{
  ...
  // The entire array will be used to build the TLAS
...
void HelloVulkan::createTopLevelAS()
{
  ...
  for (const HelloVulkan::ObjInstance& inst : m_instances)
  {
    ...
    rayInst.mask                           = 0xff; // Only be hit if rayMask and instance.mask != 0
    rayInst.instanceShaderBindingTableRecordOffset = 0; // Same hit group for all objects
    tlas.emplace_back(rayInst);
  }
  m_rtBuilder.buildTlas(tlas, VK_BUILD_ACCELERATION_STRUCTURE_PREFER_FAST_TRACE_BIT_KHR);
}

// Create descriptor set with Acceleration Structure and Output Image
...
// Write the output image to the descriptor set (required when changing resolutiom)
...
// Pipeline for ray tracer: all shaders, raygen, chit, miss
void HelloVulkan::createRtPipeline() 
{
  ...
  stage.pName = "main"; // Same entry point for all
  ...
  // Describe how the pipeline will access external data
  ...
  // Descriptor sets: 
  // - One specific to ray tracing with TLAS and output image (0) 
  // - One shared with the rasterization pipeline with scene data (1)
  ...
  // In this case, m_rtShaderGroups.size() == 3: 
  // we have one raygen group one miss shader group, and one hit group
  rayPipelineInfo.groupCount = static_cast<uint32_t>(m_rtShaderGroups.size());
  rayPipelineInfo.pGroups    = m_rtShaderGroups.data();
  rayPipelineInfo.maxPipelineRayRecursionDepth = 1; // No recursion
  rayPipelineInfo.layout                       = m_rtPipelineLayout;
  vkCreateRayTracingPipelinesKHR(m_device, {}, {}, 1, &rayPipelineInfo, nullptr, &m_rtPipeline);

  // Discard shader modules after the pipeline was created
  for(auto& s : stages)
    vkDestroyShaderModule(m_device, s.module, nullptr);
}

// Get all shader handles and write them in a SBT buffer
...
void HelloVulkan::createRtShaderBindingTable()
{
  ...
  uint32_t handleSizeAligned = nvh::align_up(handleSize, m_rtProperties.shaderGroupBaseAlignment);
